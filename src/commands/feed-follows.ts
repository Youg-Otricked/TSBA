...
export async function handlerUnfollow(cmdName: string, user: User, ...args: string[]) {
  ...
  const deleted = await unfollowFeedForUser(feed.id, user.id);
  if (!deleted) throw new Error(`You aren't following: ${feedURL}`);

  console.log(`Feed ${feedURL} successfully unfollowed`);
}